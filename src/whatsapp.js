  ...
  try {
    ...
    if (!devocionalJaEnviado) {
      ...
      if (devocionalHoje) {
        ...
        // Se não conseguiu encontrar no histórico, tentar gerar um novo
        logger.warn(`Não foi possível encontrar um devocional para enviar ao novo contato ${telefone}`);
        logger.info('Tentando gerar um novo devocional para o contato...');

        try {
          // Importar o gerador de devocional se ainda não foi importado
        ...
        // Se chegou aqui, não foi possível enviar devocional
        await wa.sendMessage(remetente, {
